...
// We find the index of the return start token
...
// Locate the system telemetery view as the safety anchor
const telemetryToken = 'function SystemTelemetryView';
const telemetryIndex = content.indexOf(telemetryToken);

if (telemetryIndex === -1) {
  console.error("Could not find SystemTelemetryView token!");
  process.exit(1);
}

// Find backwards the closing brace of IntelligenceAIView right before SystemTelemetryView
const lastCloseBraceIndex = content.lastIndexOf('}', telemetryIndex);

if (lastCloseBraceIndex === -1 || lastCloseBraceIndex < startIndex) {
  console.error("Could not find IntelligenceAIView end brace!");
  process.exit(1);
}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-5 card p-6 space-y-4 flex flex-col h-[520px]">
            ...
            <div className="space-y-1.5 flex-1 flex flex-col w-full">
              <label className="text-[9px] font-bold text-kontrol-ink-muted uppercase tracking-wider text-left block w-full flex-shrink-0">Saisissez une requête d'analyse à simuler</label>
              <textarea
            ...
            <button
              ...
              className="w-full py-3.5 bg-kontrol-dark tracking-widest text-white text-[10px] font-extrabold uppercase rounded-2xl hover:bg-kontrol-blue transition-all disabled:opacity-50 flex items-center justify-center gap-2 shadow-lg flex-shrink-0"
            >
              {isSimulating ? (
                <>
          ...
          <div className="lg:col-span-7 space-y-6 flex flex-col h-[520px]">
            ...
            {/* Answer display box */}
            <div className="h-[200px] bg-white border border-kontrol-border rounded-[2rem] p-6 overflow-y-auto flex flex-col shadow-sm text-left flex-shrink-0">
          ...
          <div className="card overflow-hidden">
            <div className="p-6 border-b border-kontrol-border bg-amber-50/20 text-left">
              <h4 className="text-xs font-extrabold text-kontrol-dark uppercase tracking-wider flex items-center justify-between w-full">
                ...
                <span className="text-[10px] font-bold text-amber-600 bg-white px-3 py-1 rounded-full border border-amber-100 font-mono">Dépenses Estimées</span>
              </h4>
            </div>
...
const targetSlice = content.substring(startIndex, lastCloseBraceIndex);
const updated = content.substring(0, startIndex) + replacement + content.substring(lastCloseBraceIndex);
fs.writeFileSync(filePath, updated, 'utf8');
console.log("SUCCESSFULLY COMPLETED COG-TOWER REWRITE WORK!");
fs.unlinkSync(__filename); // cleanup this runner
